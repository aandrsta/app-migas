<x-app-layout>
    ...
    <div class="page-title">
        <span>Edit Proyek: {{ $project->name }}</span>
    ...
    <div class="solid-card" style="max-width: 800px; margin: 0 auto;">
        ...
        <form action="{{ route('projects.update', $project) }}" method="POST" id="wizard-form">
            @csrf
            @method('PUT')
            ...
            <div class="wizard-panel active" id="panel-1">
                ...
                <div class="form-group">
                    ...
                    <input type="text" id="name" name="name" class="form-control" value="{{ old('name', $project->name) }}" required placeholder="Contoh: Lapangan Jatibarang Alpha" />
                </div>

                <div style="display: grid; grid-template-columns: repeat(auto-fit, minmax(220px, 1fr)); gap: 1.5rem;">
                    <div class="form-group">
                        <label class="form-label" for="oil_price">Harga Minyak ($ / bbl)</label>
                        <input type="number" step="any" id="oil_price" name="oil_price" class="form-control" value="{{ old('oil_price', $project->oil_price) }}" required placeholder="Contoh: 32" />
                    </div>

                    <div class="form-group">
                        <label class="form-label" for="discount_rate">Discount Rate / Suku Bunga (%)</label>
                        <input type="number" step="any" id="discount_rate" name="discount_rate" class="form-control" value="{{ old('discount_rate', $project->discount_rate) }}" required placeholder="Contoh: 10" />
                    </div>
                </div>
                ...
                <div style="display: grid; grid-template-columns: repeat(auto-fit, minmax(220px, 1fr)); gap: 1.5rem;">
                    <div class="form-group">
                        ...
                        <input type="number" step="any" id="capital_cost" name="capital_cost" class="form-control" value="{{ old('capital_cost', $project->capital_cost) }}" required placeholder="Contoh: 13000" />
                    </div>

                    <div class="form-group">
                        <label class="form-label" for="non_capital_cost">Non-Capital Cost ($M - Juta Dolar)</label>
                        <input type="number" step="any" id="non_capital_cost" name="non_capital_cost" class="form-control" value="{{ old('non_capital_cost', $project->non_capital_cost) }}" required placeholder="Contoh: 8000" />
                    </div>
                </div>
                ...
                <div style="display: grid; grid-template-columns: repeat(auto-fit, minmax(220px, 1fr)); gap: 1.5rem;">
                    <div class="form-group">
                        ...
                        <input type="number" step="any" id="opex_per_year" name="opex_per_year" class="form-control" value="{{ old('opex_per_year', $project->opex_per_year) }}" required placeholder="Contoh: 180" />
                    </div>

                    <div class="form-group">
                        <label class="form-label" for="tax_rate">Tarif Pajak Pendapatan (%)</label>
                        <input type="number" step="any" id="tax_rate" name="tax_rate" class="form-control" value="{{ old('tax_rate', $project->tax_rate) }}" required placeholder="Contoh: 51" />
                    </div>
                </div>

                <div class="form-group">
                    <label class="form-label" for="known_years">Durasi Data Produksi Aktual (Tahun)</label>
                    <input type="number" id="known_years" name="known_years" class="form-control" value="{{ old('known_years', $project->known_years) }}" required min="1" max="100" placeholder="Jumlah tahun data produksi" />
                </div>
            </div>
            ...
            <div class="wizard-panel" id="panel-2">
                ...
                <p style="color: var(--text-secondary); font-size: 0.9rem; margin-bottom: 1.5rem;">
                    Masukkan data produksi aktual dalam satuan **MBBL (Ribuan Barrel)** untuk masing-masing tahun.
                </p>
                ...
                <div class="form-group" id="reserve-group">
                    ...
                    <input type="number" step="any" id="total_reserve" name="total_reserve" class="form-control" value="{{ old('total_reserve', $project->total_reserve) }}" placeholder="Opsional (Wajib jika menggunakan metode depresiasi Unit of Production)" />
                </div>
            </div>
            ...
            <div class="wizard-panel" id="panel-3">
                ...
                <div class="form-group">
                    ...
                    <select id="depreciation_method" name="depreciation_method" class="form-control" required>
                        <option value="straight_line" {{ old('depreciation_method', $project->depreciation_method) === 'straight_line' ? 'selected' : '' }}>Straight Line Method (Garis Lurus)</option>
                        <option value="declining_balance" {{ old('depreciation_method', $project->depreciation_method) === 'declining_balance' ? 'selected' : '' }}>Declining Balance Method (Saldo Menurun)</option>
                        <option value="double_declining" {{ old('depreciation_method', $project->depreciation_method) === 'double_declining' ? 'selected' : '' }}>Double Declining Balance Method</option>
                        <option value="unit_of_production" {{ old('depreciation_method', $project->depreciation_method) === 'unit_of_production' ? 'selected' : '' }}>Unit of Production Method</option>
                        <option value="sum_of_year" {{ old('depreciation_method', $project->depreciation_method) === 'sum_of_year' ? 'selected' : '' }}>Sum of the Years Digits Method</option>
                    </select>
            ...
            <div class="wizard-navigation">
                ...
                <button type="submit" class="btn btn-success" id="btn-submit" style="display: none;">
                    <i class="fa-solid fa-circle-check"></i> Simpan & Hitung Ulang
                </button>
            </div>
        </form>
    </div>
    ...
    <script>
        document.addEventListener('DOMContentLoaded', function() {
            ...
            const oldDeclineStartYear = "{{ old('decline_start_year', $project->decline_start_year) }}";

            // Pre-existing production data from DB
            const existingProduction = {
                @foreach($actualProduction as $prod)
                    "{{ $prod->year }}": "{{ $prod->production }}",
                @endforeach
            };
            ...
            function generateProductionFields() {
                ...
                // Read current input values first before clearing
                const currentValues = {};
                ...
                for (let i = 1; i <= count; i++) {
                    const value = currentValues[i] || existingProduction[i] || '';
            ...
            // Pre-run generation so step 2 is populated when user enters it initially
            generateProductionFields();
        });
    </script>
</x-app-layout>
